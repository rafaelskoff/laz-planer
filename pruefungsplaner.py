...
def lade_daten(uploaded_file, instrument_to_jury_map=None):
    ...
    try:
        ...
        # Erkenne welche Spalte das Instrument enthält (flexibel!)
        instrument_spalte = 'instrument'  # Default
        if 'instrument_id' in df.columns and 'instrument' in df.columns:
            # Prüfe welche Spalte Namen enthält (nicht IDs)
            test_id = df['instrument_id'].dropna().unique()[:3]
            test_name = df['instrument'].dropna().unique()[:3]
            
            # Wenn instrument_id Strings mit Buchstaben hat, nutze die
            ...
            # Sonst nutze instrument wenn es Strings enthält
        ...
        df_normalized = pd.DataFrame({
            ...
            'instrument': df[instrument_spalte],  # Flexibel!
