...
class Pruefling:
    ...
    def _bestimme_jury(self, instrument_to_jury_map):
        if instrument_to_jury_map and self.instrument in instrument_to_jury_map:
            return instrument_to_jury_map[self.instrument]
        return None
...
class Scheduler:
    ...
    def __init__(self, prueflings_liste, start_zeit, jurys, jury_names=None):
...
def exportiere_zu_excel(df, jury_names, instrument_assignments):
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        ...
        for jury_name in jurys:
            ...
            # Sammle Instrumente für diese Jury
            instrumente_dieser_jury = []
            if jury_key:
                for instrument, assigned_jury_key in instrument_assignments.items():
                    if assigned_jury_key == jury_key:
    ...
    with st.sidebar:
        ...
        if st.session_state.available_instruments:
            ...
            for i in range(jury_count):
                ...
                for other_i in range(jury_count):
                    if other_i != i:
                        other_key = f"jury_{other_i}_instruments"
                        if other_key in st.session_state:
                            already_assigned.update(st.session_state[other_key])
                
                available_for_this_jury = [
                    inst for inst in st.session_state.available_instruments 
                    if inst not in already_assigned
                ]
                
                current_assigned = [
                    inst for inst in st.session_state[key]
                    if inst in st.session_state.available_instruments
                ]
                
                # Kombiniere: verfügbare + aktuell zugewiesene
                options_for_this_jury = sorted(set(available_for_this_jury + current_assigned))
                
                # WICHTIG: Kein default Parameter, nutze nur Session State
                selected = st.multiselect(
                    st.session_state.jury_names.get(jury_key, f"Jury {i+1}"),
                    options=options_for_this_jury,
                    key=key
                )
                
                for inst in selected:
                    instrument_assignments[inst] = jury_key
        else:
            instrument_assignments = {}
        ...
        if st.button("🚀 Zeitplan berechnen", type="primary", use_container_width=True):
            with st.spinner("Berechne optimierten Zeitplan..."):
                ...
                if prueflings_liste:
                    ...
                    scheduler = Scheduler(prueflings_liste, start_zeit, jurys, st.session_state.jury_names)
            ...
            for i in range(jury_count):
                ...
                if key in st.session_state:
                    for inst in st.session_state[key]:
                        export_instrument_assignments[inst] = jury_key
