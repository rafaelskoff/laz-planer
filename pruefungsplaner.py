...
def extrahiere_instrumente(df):
    ...
    Flexibel für verschiedene MLAZ-CSV-Formate
    """
    if df is None:
        return []
    
    # Versuche beide möglichen Spalten
    # Manchmal steht das Instrument in 'instrument', manchmal in 'instrument_id'
    spalte = None
    
    if 'instrument' in df.columns:
        # Prüfe ob die Spalte Namen (Strings) oder IDs (Nummern) enthält
        test_werte = df['instrument'].dropna().unique()[:5]
        if len(test_werte) > 0:
            # Wenn mindestens ein Wert ein String mit Buchstaben ist, nutze diese Spalte
            if any(isinstance(val, str) and any(c.isalpha() for c in str(val)) for val in test_werte):
                spalte = 'instrument'
    
    # Fallback zu instrument_id
    if spalte is None and 'instrument_id' in df.columns:
        test_werte = df['instrument_id'].dropna().unique()[:5]
        if len(test_werte) > 0:
            if any(isinstance(val, str) and any(c.isalpha() for c in str(val)) for val in test_werte):
                spalte = 'instrument_id'
    
    # Wenn immer noch keine Spalte gefunden, Fallback
    if spalte is None:
        if 'instrument' in df.columns:
            spalte = 'instrument'
    ...
    for inst in instrumente:
        ...
        # Filtere 'nan', leere Strings, Whitespace und reine Zahlen
        if inst_str and inst_str.lower() != 'nan' and len(inst_str) > 0:
            # Filtere reine Zahlen raus (IDs)
...
def lade_daten(uploaded_file, instrument_to_jury_map=None):
    """Lädt CSV-Datei mit automatischer Erkennung der Spalten-Struktur"""
    try:
        # Lese CSV mit BOM-Behandlung
        df = pd.read_csv(uploaded_file, sep=';', encoding='utf-8-sig')
        
        # ERKENNE CSV-FORMAT: Verschoben (alt) oder Korrekt (neu)?
        # Prüfe ob 'zuname' echte Nachnamen enthält oder Vornamen (verschoben)
        ist_verschoben = False
        if 'zuname' in df.columns and 'vorname' in df.columns:
            # Schaue ob zuname und vorname plausibel sind
            zuname_sample = df['zuname'].dropna().astype(str).head(5)
            vorname_sample = df['vorname'].dropna().astype(str).head(5)
            
            # Wenn vorname Zahlen/Daten enthält, ist es verschoben (vorname enthält dann Geburtsdatum)
            if len(vorname_sample) > 0:
                erster_vorname = str(vorname_sample.iloc[0])
                # Enthält Punkte oder Zahlen? -> Datum -> verschoben!
                if '.' in erster_vorname or erster_vorname.replace('-', '').isdigit():
                    ist_verschoben = True
        
        # Erkenne welche Spalte das Instrument enthält
        instrument_spalte = 'instrument'
        if 'instrument_id' in df.columns and 'instrument' in df.columns:
            test_id = df['instrument_id'].dropna().unique()[:3]
            test_name = df['instrument'].dropna().unique()[:3]
            
            if len(test_id) > 0 and any(isinstance(val, str) and any(c.isalpha() for c in str(val)) for val in test_id):
                instrument_spalte = 'instrument_id'
            elif len(test_name) > 0 and any(isinstance(val, str) and any(c.isalpha() for c in str(val)) for val in test_name):
                instrument_spalte = 'instrument'
        
        # Erstelle normalisiertes DataFrame basierend auf Format
        if ist_verschoben:
            # ALTES FORMAT: Spalten sind verschoben
            df_normalized = pd.DataFrame({
                'id': df['id'],
                'zuname': df['titel'],
                'vorname': df['zuname'],
                'instrument': df[instrument_spalte],
                'prüfungsbezeichnung': df['prüfungs_id'],
                'vereinsname': df['anmeldeverein_kurz'],
                'korrepetitor': df['vortrag'] if 'vortrag' in df.columns else '',
                'vorbereitungskurs_bezirk': df['prüfungsart'] if 'prüfungsart' in df.columns else 0
            })
        else:
            # NEUES FORMAT: Spalten sind korrekt benannt
            # Intelligente Korrepetitor-Erkennung
            korrepetitor_col = []
            for idx, row in df.iterrows():
                korrepert = row.get('korrepetitor', '')
                vortrag = row.get('vortrag', '')
                
                # Verwende korrepetitor wenn gefüllt und nicht leer
                if pd.notna(korrepert) and str(korrepert).strip() and str(korrepert) != 'nan':
                    korrepetitor_col.append(str(korrepert).strip())
                # Sonst: extrahiere aus vortrag wenn es "mit Klavierbegleitung" ist
                elif pd.notna(vortrag) and 'Klavierbegleitung' in str(vortrag):
                    korrepetitor_col.append('mit Klavierbegleitung')
                else:
                    korrepetitor_col.append('')
            ...
            df_normalized = pd.DataFrame({
                ...
                'vorname': df['vorname'],
                'instrument': df[instrument_spalte],
                'prüfungsbezeichnung': df['prüfungsbezeichnung'],
                'vereinsname': df['vereinsname'] if 'vereinsname' in df.columns else df.get('anmeldeverein_kurz', ''),
                'korrepetitor': korrepetitor_col,
                'vorbereitungskurs_bezirk': df.get('vorbereitungskurs_bezirk', 0)
            })
...
def erstelle_zeitplan_dataframe(zeitplan, jury_names):
    ...
    for jury_key, slots in zeitplan.items():
        ...
        for slot in slots:
            ...
            # Robuste Korrepetitor-Behandlung
            korrepetitor_wert = ''
            if not p.ist_pause and p.korrepetitor:
                korrepetitor_wert = str(p.korrepetitor).strip()
                if korrepetitor_wert.lower() in ['none', 'nan', '']:
                    korrepetitor_wert = ''
            ...
            data.append({
                ...
                'Korrepetitor': korrepetitor_wert
            })
        ...
        for jury_name in jurys:
            ...
            for idx, row in jury_df.iterrows():
                if row['Name'] == PAUSE_MARKER:
                    ...
                    # Robuste Behandlung von Korrepetitor (verhindere None/NaN)
                    korrepetitor_wert = row.get('Korrepetitor', '')
                    if pd.isna(korrepetitor_wert) or korrepetitor_wert is None:
                        korrepetitor_wert = ''
                    else:
                        korrepetitor_wert = str(korrepetitor_wert).strip()
                        if korrepetitor_wert.lower() == 'none' or korrepetitor_wert.lower() == 'nan':
                            korrepetitor_wert = ''
                    
                    export_data.append({
                        'Beginnuhrzeit': row['Start'],
                        'Name': row['Name'],
                        'Musikverein': row['Musikverein'] if row['Musikverein'] else '',
                        'Instrument': row['Instrument'] if row['Instrument'] else '',
                        'Stufe': row['Stufe'] if row['Stufe'] else '',
                        'Bemerkung': korrepetitor_wert
                    })
            ...
            for row_num, row_data in enumerate(export_data, start=4):
                ...
                # Schreibe Zellen in korrekter Reihenfolge (konvertiere zu String!)
                worksheet.write(row_num, 0, str(row_data['Beginnuhrzeit']), time_fmt)
                worksheet.write(row_num, 1, str(row_data['Name']), row_format)
                worksheet.write(row_num, 2, str(row_data['Musikverein']), row_format)
                worksheet.write(row_num, 3, str(row_data['Instrument']), row_format)
                worksheet.write(row_num, 4, str(row_data['Stufe']), row_format)
                worksheet.write(row_num, 5, str(row_data['Bemerkung']), row_format)
...
def main():
    ...
    with st.sidebar:
        ...
        uploaded_file = st.file_uploader(
            "CSV-Datei hochladen",
            type=['csv'],
            help="MLAZ-Export oder Backup-Zeitplan (CSV)"
        )
        ...
        if uploaded_file is not None:
            ...
            if st.session_state.uploaded_df is None or uploaded_file.name != st.session_state.get('last_uploaded_filename'):
                try:
                    ...
                    # ERKENNE TYP: MLAZ-Original oder Backup-Zeitplan?
                    ist_backup = False
                    if 'Jury/Raum' in df_temp.columns and 'Dauer (min)' in df_temp.columns:
                        # Das ist ein exportierter Zeitplan (Backup)!
                        ist_backup = True
                        st.session_state.uploaded_df = df_temp
                        st.session_state.last_uploaded_filename = uploaded_file.name
                        
                        # Lade direkt als Zeitplan
                        st.session_state.schedule = df_temp
                        
                        # Extrahiere Instrumente aus dem Zeitplan
                        instrumente_im_zeitplan = df_temp['Instrument'].dropna().unique().tolist()
                        instrumente_clean = [str(i) for i in instrumente_im_zeitplan if str(i) != 'nan' and str(i) != '-']
                        st.session_state.available_instruments = sorted(instrumente_clean, key=lambda x: x.lower())
                        
                        # Erkenne Jury-Anzahl
                        jurys_im_backup = df_temp['Jury/Raum'].unique()
                        st.session_state.jury_count = len(jurys_im_backup)
                        
                        # Setze Jury-Namen
                        for jury_name in jurys_im_backup:
                            # Finde Jury-Key
                            for i in range(10):
                                jury_key = f"Jury_{i+1}"
                                if st.session_state.jury_names.get(jury_key, f"Jury {i+1}") == jury_name or f"Jury {i+1}" == jury_name:
                                    st.session_state.jury_names[jury_key] = jury_name
                                    break
                        
                        uploaded_file.seek(0)
                        st.success(f"✅ Backup geladen: {len(df_temp)} Einträge, {len(jurys_im_backup)} Jurys")
                        st.info("💾 **Backup-Zeitplan geladen** - Sie können direkt mit der Bearbeitung fortfahren!")
                    else:
                        # Normaler MLAZ-Export
                        st.session_state.uploaded_df = df_temp
                        st.session_state.last_uploaded_filename = uploaded_file.name
    ...
    if st.session_state.schedule is not None:
        ...
        # Zeitplan speichern/laden
        st.subheader("💾 Zeitplan speichern & laden")
        
        col_save, col_load = st.columns(2)
        
        with col_save:
            st.markdown("**Zeitplan speichern:**")
            if st.session_state.schedule is not None:
                # Erstelle Pickle-Datei mit allen Daten
                import pickle
                save_data = {
                    'schedule': st.session_state.schedule,
                    'jury_names': st.session_state.jury_names,
                    'jury_count': jury_count,
                    'available_instruments': st.session_state.available_instruments,
                    'instrument_assignments': {}
                }
                
                # Sammle Instrumenten-Zuordnungen
                for i in range(jury_count):
                    key = f"jury_{i}_instruments"
                    if key in st.session_state:
                        save_data['instrument_assignments'][key] = st.session_state[key]
                
                # Serialisiere
                save_bytes = pickle.dumps(save_data)
                
                timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M")
                st.download_button(
                    label="💾 Zeitplan als Backup speichern",
                    data=save_bytes,
                    file_name=f"Zeitplan_Backup_{timestamp}.pkl",
                    mime="application/octet-stream",
                    help="Speichert den kompletten Zeitplan inkl. Pausen und Einstellungen",
                    use_container_width=True
                )
            else:
                st.info("ℹ️ Erst Zeitplan berechnen, dann speichern")
        
        with col_load:
            st.markdown("**Zeitplan laden:**")
            backup_file = st.file_uploader(
                "Backup-Datei (.pkl) hochladen",
                type=['pkl'],
                help="Lädt einen zuvor gespeicherten Zeitplan",
                key="backup_uploader"
            )
            ...
            if backup_file is not None:
                try:
                    import pickle
                    loaded_data = pickle.load(backup_file)
                    
                    # Restore State
                    st.session_state.schedule = loaded_data['schedule']
                    st.session_state.jury_names = loaded_data['jury_names']
                    st.session_state.available_instruments = loaded_data.get('available_instruments', [])
                    
                    # Restore Instrumenten-Zuordnungen
                    for key, value in loaded_data.get('instrument_assignments', {}).items():
                        st.session_state[key] = value
                    
                    st.success("✅ Zeitplan erfolgreich geladen!")
                    st.rerun()
                except Exception as e:
                    st.error(f"❌ Fehler beim Laden: {str(e)}")
